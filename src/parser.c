...
void parseRender(render_t *render_info, char *line) {
  ...
  render_info->buffer = newFrameBuffer(width, height, depth);
  render_info->fps = fps;
...
void parseCamera(camera_t **cam, char *line) {
  ...
  *cam = newCamera(from, to, up, fov, aspect_ratio, aperture, focus_dist);
}
...
void parseMaterial(scene_t *sc, hashtable_t *materials, char *line) {
  ...
  if (strcmp(mat_type, "metal") == 0) {
    mat = createMetal(sc, mat_albedo, fuzz);
  } else if (strcmp(mat_type, "lambertian") == 0) {
    mat = createLambertian(sc, mat_albedo);
  } else if (strcmp(mat_type, "glass") == 0) {
    mat = createGlass(sc, mat_albedo, ir);
  }
...
void parseOrbit(scene_t *sc, const render_t *render_info,
  ...
  newOrbitAnimation(sc->animations, target, pivt, y, degreesToRadians(angle),
