...
animlist_t *newAnimlist() { return newList(); }
...
void freeAnimlist(animlist_t *animlist) {
  freeList(animlist, (val_free_func)&animation_free);
}

void animlist_animate(animlist_t *animlist, double time) {
  double end_time;
  for (animation_t *a = (animation_t *)listIterate(animlist); a != NULL;
       a = (animation_t *)listIterate(NULL)) {
...
va_t *newValueAnimation(animlist_t *animlist, double *value, double from,
                        double to, double delay, double duration,
...
void orbit_animate(animation_t *orbit_animation, double time) {
  ...
  vec3_t x = mulVec3(oa_data->x, cos(current_angle));
  vec3_t y = mulVec3(oa_data->y, sin(current_angle));
  *target = addVec3(*pivot, addVec3(x, y));
}

orbitanim_t *newOrbitAnimation(animlist_t *animlist, vec3_t *target,
                               vec3_t *pivot, vec3_t y, double angle,
  ...
  oa_data->pivot = pivot;
  oa_data->x = subVec3(*target, *pivot);
  oa_data->y = y;
  oa_data->angle = angle;
