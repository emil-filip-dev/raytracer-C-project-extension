...
void testParseCamera(void) {
  ...
  camera_t *sol = newCamera(look_from, look_at, vup, vfov, aspect_ratio,
                            aperture, focus_dist);
  assert(equalsVec3(cam->origin, sol->origin));
  assert(equalsVec3(cam->lower_left_corner, sol->lower_left_corner));
  assert(equalsVec3(cam->horizontal, sol->horizontal));
  assert(equalsVec3(cam->vertical, sol->vertical));
  assert(equalsVec3(cam->u, sol->u));
  assert(equalsVec3(cam->v, sol->v));
  assert(equalsVec3(cam->w, sol->w));
  assert(doubleEquals(cam->lens_radius, sol->lens_radius));
  free(cam);
  free(sol);
}
...
void testParseSphere(void) {
  ...
  assert(equalsVec3(h->position, pos));
  sphere_data *sd = (sphere_data *)h->object_data;
  assert(doubleEquals(sd->radius, 6.0));
...
void testParseOrbit(void) {
  ...
  assert(doubleEquals(anim->delay, 0));
  assert(doubleEquals(anim->duration, 1));
  ...
  assert(equalsVec3(orbAnim->y, y));
  assert(doubleEquals(orbAnim->angle, degreesToRadians(1080)));
}
